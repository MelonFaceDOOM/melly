...
from app import db
from app.main.forms import EditProfileForm, PostForm, CreateCategoryForm
...
def index():
    ...
    if form.validate_on_submit():
        post = Post(body=form.post.data, author=current_user) #todo - add thread_id
        db.session.add(post)
        db.session.commit()
    ...
    return render_template('index.html', title='Home', form=form,
                           posts=posts.items, categories=categories.items, next_url=next_url,
                           prev_url=prev_url)
...
def create_category():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('main.index')) #update with url for new category?
